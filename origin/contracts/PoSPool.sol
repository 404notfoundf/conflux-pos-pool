...
// File: contracts/11.self_cspace.sol
...
contract PoSPool is PoolContext, Ownable, Initializable {
  ...
  function register(
    ...
    userSummaries[msg.sender].locked += votePower;  // directly add to admin locked votes
  ...
  ///
  function claimInterest(uint amount) public onlyRegistered {
  ...
  ///
  function claimAllInterest() public onlyRegistered {
    uint claimableInterest = userInterest(msg.sender);
  ...
  function _withdrawPoolProfit(uint256 amount) public onlyOwner {
    require(_poolSummary.interest > amount, "Insufficient Pool Interest");
    require(_selfBalance() > amount, "Insufficient Pool Balance");
