...
contract DxCFXBridge is Ownable, Initializable, SpaceBridge {
    ...
    function withdrawPoolInterest(uint256 amount) public onlyOwner {
        require(amount <= poolAccInterest, "DxCFXBridge: insufficient pool interest");
    ...
    function decreaseStake(uint64 votes) public onlyOwner {
        ...
        require(userSummary.locked >= votes, "DxCFXBridge: insufficient votes");
    ...
    function unstakeVotes(uint64 _votes) public onlyOwner {
        IPoSPool.UserSummary memory userSummary = poolSummary();
        require(userSummary.locked >= _votes, "DxCFXBridge: insufficient votes");
