 ...
 * that only accounts with this role will be able to grant or revoke standard
...
abstract contract AccessControlUpgradeable is Initializable, ContextUpgradeable, IAccessControlUpgradeable, ERC165Upgradeable {
     ...
     *
     * Using this function in any standard way is effectively circumventing the admin
 ...
 * and pauser roles to standard accounts.
 *
...
contract DxCFX is ERC20PresetMinterPauserUpgradeable {
    ...
    address public coreBridge; // core bridge address

    uint256 public totalStake; // total stake of CFX in this pool
    uint256 public totalRedeemed; // total redeemed, which is need full filled from CoreBridge
    ...
    event Redeem(address user, uint256 share, uint256 amount);
    event Withdraw(address user, uint256 amount);

    function initialize() public initializer {
        super.initialize("DxPool CFX", "DxCFX");
    ...
    function deposit() public payable {
        ...
        if (totalSupply() == 0) {
            ...
            shares = (depositAmount * totalSupply()) / totalStake;
        }

        _mint(msg.sender, shares);
        totalStake += depositAmount;
        _transferToBridge(depositAmount);
    ...
    function redeem(uint256 shares) public {
        ...
        uint256 amount = (shares * totalStake) / totalSupply();
        _burn(msg.sender, shares);
        totalStake -= amount;

        _redeemQueue.enqueue(RedeemQueue.Node({amount: amount, user: msg.sender})); // enter redeem queue

        userRedeemed[msg.sender] += amount;
    ...
    function withdraw(uint256 amount) public {
        ...
        require(amount <= userWithdrawable[msg.sender], "insufficient withdrawable amount");
    ...
    */
    function cfxRatio() public view returns (uint256) {
        ...
        return totalStake * RATIO_BASE / totalSupply();
    }
    ...
    function handleRedeem() public payable onlyBridge {
        ...
        RedeemQueue.Node memory redeemNode = _redeemQueue.dequeue();
        require(redeemNode.amount == value, "redeem amount not match");
        require(value <= totalRedeemed, "redeem abnormal");
        totalRedeemed -= value;

        userWithdrawable[redeemNode.user] += value;
        userRedeemed[redeemNode.user] -= value;
    }

    function addStake(uint256 delta) public onlyBridge {
        totalStake += delta;
    }
